...
<x-app-layout>
      ...
      <div class="m-3 align-self-center">
        <div x-data="{ open: false }" @click.away="open = false" class="relative inline-block text-left">
          <button @click="open = !open" type="button"
            class="inline-flex items-center justify-center w-full px-4 py-2 text-sm font-medium text-white  bg-yellowB border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
            Choose date
            <div class="ml-1 ">
          ...
          <div x-show="open"
            class="absolute right-0 z-10 mt-2 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md">
        ...
        <div x-data="{ open: false }" @click.away="open = false" class="relative inline-block text-left">
          ...
          <div x-show="open"
            class="absolute right-0 z-10 mt-2 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md">
            <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
              <a href="{{route('home.booking.floor2')}}"
                class="block px-4 py-2 text-sm text-black hover:bg-gray-100 hover:text-gray-900 no-underline"
                role="menuitem">Floor 2</a>
            </div>
          </div>
        </div>
      </div>
  ...
  @for ($i = 0; $i < 6; $i++) <div class="modal-wrapper" data-modal-id="{{ $i }}">
    ...
    <div class="second-modal fixed inset-0 hidden">
      <div class="flex modals modal-contents absolute bg-grey p-8 rounded-lg w-72 justify-center items-center flex-col">
        ...
        <div class="flex flex-row"><button
            class="custom-a flex items-center justify-center open-third-modal">Yes</button> {{-- Include logic here,
          dapat masave sa db once click --}}
          <button class="custom-a flex items-center justify-center close">No</button>
        </div>
    ...
    <div class="third-modal fixed inset-0 hidden">
      <div class="flex modals modal-contents absolute bg-grey p-8 rounded-lg w-72 justify-center items-center flex-col">
        <span class="close absolute -top-6 right-2 text-white cursor-pointer">X</span>
        <p class="text-xl text-center">Booking# successfully <span class="text-yellowB">placed</span></p> {{-- Include automated booking# here --}}
        <h6 class="text-xl">DATE - (Status)</h6> {{-- Include automated date here --}}
        <p class="text-lg font-semibold -mb-0">Desk#</p> {{-- Include automated desk here --}}
        <h6 class="mb-4">Booked by: {{ Auth::user()->name }}</h6>
        <a href='{{route('dashboard')}}'
          class="flex items-center justify-center w-40 bg-yellowB text-white rounded-lg no-underline py-1">Back to
          Home</a>
